...
function speedOf(stage){return stage.querySelector('[data-bv-speed]')?.textContent?.includes('2')?2:1;}
...
function animateWuji(stage){
  ...
  const speed=speedOf(stage),root=stage.getBoundingClientRect();
  ...
  portrait.animate([
    ...
    {transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'}
  ],{duration:720/speed,easing:'cubic-bezier(.18,.78,.2,1)'});
  ...
  [0,-18,18].forEach((offset,i)=>{
    ...
    beam.style.setProperty('--delay',`${(95+i*42)/speed}ms`);
    beam.style.setProperty('--dur',`${(300+i*25)/speed}ms`);
  ...
  wave.style.setProperty('--dur',`${360/speed}ms`);
  ...
  later(()=>{
    ...
    stage.animate([
      ...
      {transform:'translate3d(0,0,0)'}
    ],{duration:210/speed,easing:'ease-out'});
  },240/speed);

  later(()=>removeFx(stage),760/speed);
}
