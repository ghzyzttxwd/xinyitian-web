...
function enemyNamesFromLog(lines,title){
  ...
  return Array.from({length:3},(_,i)=>`${label}${i+1}`);
}
...
function fighterCard(f){
  ...
  return `<div class="bv-fighter ${f.side}" data-fighter-name="${esc(f.name)}" data-fighter-id="${esc(f.id)}"><div class="bv-portrait"><span>${esc(initial)}</span></div><div class="bv-name">${esc(f.name)}</div><div class="bv-hp"><i style="width:100%"></i></div><div class="bv-rage"><i style="width:25%"></i></div></div>`;
}
...
function buildController(stage,events,players,enemies,finalBox,outcome,reward){
  ...
  function floatText(el,text,kind='damage'){if(!el)return;const n=document.createElement('span');n.className=`bv-float ${kind}`;n.textContent=text;el.appendChild(n);setTimeout(()=>n.remove(),780/Math.max(1,speed));}
  function setTicker(text){ticker.textContent=text||'';}
  ...
  function applyAction(event){
    ...
    const actorEl=card(actor.name);pulse(actorEl,event.normal?'acting':'skill-cast',event.normal?430:620);
    ...
    if(event.skill){skillEl.textContent=`${actor.name} · ${event.skill}`;skillEl.classList.add('show');setTimeout(()=>skillEl.classList.remove('show'),650/Math.max(1,speed));}
    ...
    for(const t of targets){
      const el=card(t.name);pulse(el,'hit',430);
  ...
  function applyEvent(event){if(event.type==='round'){roundEl.textContent=`第 ${event.round} 回合`;setTicker(`第${event.round}回合`);return;}if(event.type==='action')return applyAction(event);if(event.type==='notice')return applyNotice(event);}
  function delayFor(event){if(event.type==='round')return 360/speed;if(event.type==='action')return (event.normal?560:760)/speed;return 420/speed;}
  ...
  function finish(){if(finished)return;finished=true;if(timer)clearTimeout(timer);const win=outcome.classList.contains('battle-win');const losing=win?enemies:players;for(const f of losing){f.alive=false;f.hpPct=0;sync(f);}roundEl.textContent=win?'战斗胜利':'战斗失败';setTicker(win?'胜利！奖励已经结算。':'战败，可调整阵容与养成后再试。');finalBox.hidden=false;finalBox.classList.add('show');stage.querySelector('[data-bv-skip]')?.setAttribute('disabled','');}
  ...
  function stop(){if(timer)clearTimeout(timer);finished=true;}
...
function enhanceBattleDialog(){
  ...
  const stage=document.createElement('section');stage.className='battle-visual-stage';stage.innerHTML=`<div class="bv-stage-top"><span data-bv-round>准备战斗</span><div class="bv-controls"><button class="btn btn-small" type="button" data-bv-speed>1×</button><button class="btn btn-small" type="button" data-bv-skip>跳过</button></div></div><div class="bv-skill-banner" data-bv-skill></div><div class="bv-field"><div class="bv-team player-team">${players.map(fighterCard).join('')}</div><div class="bv-center-mark"><b>VS</b></div><div class="bv-team enemy-team">${enemies.map(fighterCard).join('')}</div></div><div class="bv-ticker" data-bv-ticker>双方侠客入场……</div>`;
...
// 修正设置页旧版本文本；页面每次重绘都自动同步。
const page=document.querySelector('#page');
const versionObserver=new MutationObserver(()=>{for(const el of page?.querySelectorAll('.game-info .hero-meta')||[])if(/V0\.\d+/.test(el.textContent))el.textContent=el.textContent.replace(/V0\.\d+(?:\.\d+)?/,'V0.23.0');});
if(page)versionObserver.observe(page,{childList:true,subtree:true});
