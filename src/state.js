...
function mergeDefaults(saved, fresh) {
  ...
  if (saved && typeof saved === 'object') {
    Object.keys(saved).forEach(key => {
      out[key] = key in fresh ? mergeDefaults(saved[key], fresh[key]) : saved[key];
    });
  }
  return out;
}
...
export function recoverStamina(state) {
  const now = Date.now();
  const last = Number(state.player.lastStaminaAt || now);
...
export function recalcVip(state) {
  ...
  for (let i = 1; i < VIP_THRESHOLDS.length; i++) {
    if (state.player.totalRecharge >= VIP_THRESHOLDS[i]) vip = i;
    else break;
  }
...
export function heroPower(state, heroId) {
  const s = heroStats(state, heroId);
  if (!s) return 0;
  return Math.round(s.atk * 4.6 + s.def * 3.5 + s.hp * .7 + s.speed * 6);
}

export function totalPower(state) {
  return state.party.filter(Boolean).reduce((sum, id) => sum + heroPower(state, id), 0);
}
...
export function exportSave(state) {
  ...
  a.href = url;
  a.download = `新倚天存档-${localDateKey()}.json`;
  a.click();
...
export async function importSaveFile(file) {
  ...
  const state = mergeDefaults(parsed, createInitialState());
  normalizeDaily(state);
  recoverStamina(state);
  recalcVip(state);
  saveState(state);
