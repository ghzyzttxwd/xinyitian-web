...
function buildController(stage,events,players,enemies,finalBox,outcome){
  ...
  function dashAttack(actorEl,targetEl,mode='normal'){
    ...
    stage.appendChild(ghost);source.style.visibility='hidden';
    ...
    if(ghost.animate){
      const frames=mode==='wudang'?
        [{transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'},{offset:.16,transform:'translate3d(-8px,2px,0) scale(.96)',filter:'brightness(1.08)'},{offset:.48,transform:`translate3d(${dx*.78}px,${dy*.78-3}px,0) scale(1.08)`,filter:'brightness(1.35) drop-shadow(0 0 8px rgba(255,224,155,.7))'},{offset:.62,transform:`translate3d(${dx*.9}px,${dy*.9}px,0) scale(1.12)`,filter:'brightness(1.6) drop-shadow(0 0 12px rgba(255,211,118,.9))'},{offset:.78,transform:`translate3d(${dx*.72}px,${dy*.72}px,0) scale(1.04)`,filter:'brightness(1.12)'},{transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'}]
        :[{transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'},{offset:.18,transform:`translate3d(${dx*.18}px,${dy*.18-4}px,0) scale(1.08)`,filter:'brightness(1.25)'},{offset:.52,transform:`translate3d(${dx*.82}px,${dy*.82}px,0) scale(1.15)`,filter:'brightness(1.45)'},{offset:.68,transform:`translate3d(${dx*.72}px,${dy*.72}px,0) scale(1.08)`,filter:'brightness(1.15)'},{transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'}];
      const anim=ghost.animate(frames,{duration:scaled(mode==='wudang'?620:480),easing:'cubic-bezier(.2,.8,.2,1)'});anim.onfinish=finish;anim.oncancel=finish;
  ...
  function wudangLongFist(actorEl,targetEl,crit){
    ...
    setTimeout(()=>{addImpact(targetEl,'wudang-fist',crit);stageShake(crit);},scaled(360));
  }
  ...
  function applyAction(event){
    ...
    if(event.normal){const targetEl=card(targets[0].name);dashAttack(actorEl,targetEl);setTimeout(()=>{addImpact(targetEl,'slash',crit);stageShake(crit);settle();},scaled(190));}
    else if(isWujiLongFist){const targetEl=card(targets[0].name);wudangLongFist(actorEl,targetEl,crit);setTimeout(settle,scaled(360));}
  ...
  function delayFor(event){if(event.type==='round')return scaled(330);if(event.type==='action')return scaled(event.normal?650:850);return scaled(420);}
...
function enhanceBattleDialog(){
  ...
  const stage=document.createElement('section');stage.className='battle-visual-stage battle-v240';stage.dataset.bvSpeedLevel='1';stage.innerHTML=`<div class="bv-stage-top"><span data-bv-round>准备战斗</span><div class="bv-controls"><button class="btn btn-small" type="button" data-bv-speed>1×</button><button class="btn btn-small" type="button" data-bv-skip>跳过</button></div></div><div class="bv-skill-banner" data-bv-skill></div><div class="bv-field"><div class="bv-team player-team">${players.map((f,i)=>fighterCard(f,i)).join('')}</div><div class="bv-center-mark"><b>VS</b></div><div class="bv-team enemy-team">${enemies.map((f,i)=>fighterCard(f,i)).join('')}</div></div><div class="bv-ticker" data-bv-ticker>双方侠客入场……</div>`;
