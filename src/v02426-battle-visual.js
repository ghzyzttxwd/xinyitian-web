...
function buildController(stage,events,players,enemies,finalBox,outcome){
  ...
  function applyAction(event){
    ...
    if(event.normal){const targetEl=card(targets[0].name);dashAttack(actorEl,targetEl);setTimeout(()=>{addImpact(targetEl,'slash',crit);stageShake(crit);settle();},scaled(205));}
    ...
    else if(targets.length===1){const targetEl=card(targets[0].name);singleSkillCast(actorEl,targetEl,crit);setTimeout(settle,scaled(350));}
  ...
  function delayFor(event){if(event.type==='round')return scaled(330);if(event.type==='action')return scaled(event.normal?680:900);return scaled(420);}
  ...
  function stop(){if(timer)clearTimeout(timer);finished=true;for(const ghost of stage.querySelectorAll('.bv-dash-ghost,.bv-impact,.bv-skill-wave,.bv-skill-aura,.bv-single-skill-aura,.wuji-longfist-breath'))ghost.remove();}
