...
function buildController(stage,events,players,enemies,finalBox,outcome){
  ...
  // V0.24.29: event-driven Zhang Wuji single-target punch.
  // Movement only controls position. The real APK att1 sprite controls contact and strike completion.
  function wujiPunchAttack(actorEl,targetEl,mode='normal',onImpact=null){
    ...
    const startStrike=()=>{
      ...
      requestAnimationFrame(()=>{
        ...
        // Fallback only: the frame script normally emits exact contact/end events.
  ...
  function applyAction(event){
    ...
    const isWujiLongFist=!event.normal&&isWuji&&event.skill==='武当长拳';
    if(event.normal){
    ...
    else if(isWujiLongFist){const targetEl=card(targets[0].name);wudangLongFist(actorEl,targetEl,crit,settle);}
    else if(targets.length===1){
      const targetEl=card(targets[0].name);
      if(isWuji)wujiSingleSkillCast(actorEl,targetEl,crit,settle);
      else{singleSkillCast(actorEl,targetEl,crit);setTimeout(settle,scaled(350));}
    }
  ...
  function stop(){
    if(timer)clearTimeout(timer);finished=true;
