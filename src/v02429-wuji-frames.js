// V0.24.29: Zhang Wuji original APK frame choreography.
// Event-driven phases: travel -> strike -> return. No impact +/- timer guessing.
// Damage timing is emitted from the real att1 contact frame; return starts only after att1 completes.
const battleBody=document.querySelector('#battleDialogBody');
if(!battleBody) throw new Error('battle body missing');

const PARTS=['00','01','02a','02b','03','04','05'].map(id=>`./assets/original/wuji-frames/atlas.part.${id}.txt?v=02429`);
...
const ACTIONS={
  ...
  att1:{loop:false,interval:45,contactFrame:5,frames:[
...
function play(node,actionName,{hold=false,onDone=null,onFrame=null}={}){
  ...
  const interval=()=>Math.max(22,action.interval/speedRate(stage));
  const step=()=>{
    if(states.get(node)?.token!==token||!node.isConnected)return;
    const frameIndex=i++;
    setFrame(node,actionName,frameIndex);
    onFrame?.(frameIndex);
    if(i>=action.frames.length){
      if(action.loop){i=0;state.timer=setTimeout(step,interval());return;}
...
function bindFighter(fighter){
  if(!fighter||fighter.dataset.wujiFramesBound==='029')return;
  fighter.dataset.wujiFramesBound='029';
...
function applyGhostPhase(ghost){
  if(!ghost?.isConnected)return;
  const phase=ghost.dataset.wujiPhase||'travel';
  if(phase==='strike'){
    delete ghost.dataset.wujiContactSent;
    play(ghost,'att1',{
      onFrame:index=>{
        if(index!==ACTIONS.att1.contactFrame||ghost.dataset.wujiContactSent==='1')return;
        ghost.dataset.wujiContactSent='1';
        ghost.dispatchEvent(new CustomEvent('xyt-wuji-strike-contact'));
      },
      onDone:()=>{
        if(!ghost.isConnected||ghost.dataset.wujiPhase!=='strike')return;
        ghost.dispatchEvent(new CustomEvent('xyt-wuji-strike-end'));
      }
    });
    return;
  }
  // Travel, arrival hold and return are always stable stand frames.
  // In particular, entering return cancels att1 immediately so no punch can leak into the way back.
  play(ghost,'stand');
}
function bindGhost(ghost){
  if(!ghost||ghost.dataset.wujiFramesBound==='029')return;
  ghost.dataset.wujiFramesBound='029';
  ghost.classList.add('wuji-frame-ready');
  applyGhostPhase(ghost);
  const observer=new MutationObserver(()=>applyGhostPhase(ghost));
  observer.observe(ghost,{attributes:true,attributeFilter:['data-wuji-phase']});
}
