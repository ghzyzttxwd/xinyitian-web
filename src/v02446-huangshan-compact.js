...
function idleMagic(){const run=()=>preloadMagic().catch(e=>console.warn('[XYT] Huangshan compact ultimate prep failed; fallback stays active.',e));if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:2500});else setTimeout(run,900)}
function bindFighter(f){if(!f||f.dataset.huangshanFrames==='046')return;f.dataset.huangshanFrames='046';react(f);idleMagic();const o=new MutationObserver(()=>react(f));o.observe(f,{attributes:true,attributeFilter:['class']})}
...
loadSequential(['stand','run','att1','att2','hit1']).then(()=>{roleReady=true;scan();idleMagic()}).catch(e=>console.warn('[XYT] Huangshan role resize failed; generic fallback retained.',e));
