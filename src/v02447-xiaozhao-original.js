...
function bindFighter(f){if(!f||f.dataset.xiaozhaoFrames==='047')return;f.dataset.xiaozhaoFrames='047';react(f);const o=new MutationObserver(()=>react(f));o.observe(f,{attributes:true,attributeFilter:['class']})}
...
loadSequential(['stand','run','att1','att2','hit1']).then(()=>{roleReady=true;scan()}).catch(e=>console.warn('[XYT] Xiao Zhao role resize failed; generic fallback retained.',e));
